...
export function CommandPaletteDemo() {
  ...
  return (
    <div className="w-full">
      <div
        className={`border border-border bg-card/80 overflow-hidden transition-all duration-300 ${
          isOpen ? "opacity-100 scale-100" : "opacity-50 scale-[0.98]"
        }`}
      >
        {/* Search input */}
        ...
        <div className="max-h-[160px] overflow-y-auto">
          {filtered.map((item, i) => (
            <div
              key={item.label}
              className={`flex items-center justify-between px-3 py-2 transition-colors duration-150 cursor-default ${
                i === selectedIndex
        ...
        {/* Footer hints */}
        <div className="flex items-center gap-3 px-3 py-1.5 border-t border-border/60 bg-secondary/20">
      ...
      <div className="border border-border bg-card/80 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-secondary/20">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="px-1.5 py-0.5 font-mono text-[9px] border border-border/60 text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors"
            >
              undo
            </button>
            <button
        ...
        <div className="flex items-center justify-between px-3 py-1.5 border-t border-border/60 bg-secondary/20">
          <span className="font-mono text-[8px] text-muted-foreground/50">
            {content.length} chars | history: {historyIndex + 1}/{history.length}
          </span>
          {lastAction && (
            <span
          ...
          <div className="flex items-center gap-2">
            <kbd className="font-mono text-[8px] text-muted-foreground/40 px-1 border border-border/40">
              Cmd+Z
            </kbd>
            <kbd className="font-mono text-[8px] text-muted-foreground/40 px-1 border border-border/40">
              Cmd+S
  ...
  const navigate = useCallback(
    (pageId: string) => {
      if (pageId !== currentPage) {
        ...
        setTrail((prev) => [...prev.slice(-3), pageId]);
      }
    },
    [currentPage]
  );
...
export function RecordingDemo() {
  ...
  return (
    <div className="w-full">
      <div
        className="border border-border bg-card/80 overflow-hidden"
        onKeyDown={handleKeyDown}
        tabIndex={0}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-secondary/20">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-muted-foreground">
              {isRecording ? "recording... press any combo" : "keybind recorder"}
            </span>
          </div>
          <button
        ...
        {/* Current recording */}
        <div className="flex items-center justify-center px-3 py-4 min-h-[56px]">
          {isRecording && currentKeys.length > 0 ? (
            <div className="flex items-center gap-1">
              {currentKeys.map((key, i) => (
                <span key={i} className="flex items-center gap-1">
                  {i > 0 && <span className="text-muted-foreground/30 text-xs">+</span>}
                  <kbd className="font-mono text-sm text-primary px-2 py-1 border border-primary/30 bg-primary/5">
                    {key}
                  </kbd>
                </span>
              ))}
